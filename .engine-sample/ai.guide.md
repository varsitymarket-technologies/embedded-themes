# Guideline for Recreating the `.engine-sample/interface` Template as Shopify HTML Templates

---

## Goal
Create a comprehensive **guideline.md** that documents how to transform the existing `/.engine-sample/interface` HTML/CSS/JS template into a set of **Shopify-compatible HTML (Liquid) templates**. The guide should cover:
...
:root {
  ...
  --crimson: #e8000f;
  --ember:   #ff4444;
  --glow:    rgba(192,0,10,0.15);
}
```
These should be exported as **Shopify theme settings** (via `settings_schema.json`) so merchants can customize colors.
...
### 2.1 Layout
Create `layout/theme.liquid` that includes:
```liquid
<!doctype html>
<html lang="en">
<head>
...
| Checkout (`view-checkout`) | `templates/checkout.liquid` (Shopify handles most) | Custom `checkout-steps` if needed |
...
- Replace static links with `{{ routes.root_url }}` or `{{ routes.collections_url }}`.
- Add a **mobile navigation** toggle using the same HTML but with Shopify‑compatible IDs.
- Export navigation colors as theme settings.
...
```
- Use Shopify's `product` object.
- Wishlist functionality can be kept via localStorage (no server side needed).

### 3.5 Cart Drawer (`sections/cart-drawer.liquid`)
- Keep the same markup but replace JavaScript calls with **Shopify Ajax API** (`/cart/add.js`, `/cart/change.js`).
- Update `cart.updateBadge()` to read from `Shopify.cart`.

### 3.6 Checkout Steps (`sections/checkout-steps.liquid`)
- Shopify already provides checkout UI; if a custom pre‑checkout page is needed, use `templates/checkout.liquid` and embed the same steps.
...
- Replace CSS variables with **Shopify theme settings** using the `{{ settings.color_primary }}` syntax (e.g., `--blood: {{ settings.color_primary }}`).

### 4.2 Fonts
- The template loads Google Fonts (`Bebas Neue`, `Barlow`, `IBM Plex Mono`). Add them to `theme.liquid` head via `{{ 'fonts.css' | asset_url | stylesheet_tag }}` or directly link to Google.

### 4.3 Icons & SVGs
- Store SVG icons in `assets/` and reference with `{{ 'icon-search.svg' | asset_url }}`.
- For dynamic icons (e.g., social), create snippets.
...
### 5.1 Asset `theme.js`
- Copy the existing `<script>` content (mock API, cart, auth, wishlist) into `assets/theme.js`.
- Replace any direct DOM queries with **Shopify-specific selectors** (`{{ 'cart' | json }}`) where possible.
- Keep the **mock data** only for development; in production the Ajax API will replace it.

### 5.2 Shopify Ajax Cart
Replace custom cart methods with Shopify Ajax calls:
```js
...
```
- Update `cart.updateBadge()` to read from `/cart.js`.
...
- Add `aria-label`s to navigation buttons and form fields.
...
```
- Map each CSS variable to a setting and reference in `theme.css`.
...
4. In the **Theme Editor**, customize colors, logo, and hero image using the settings created.
...
If you need server‑side features (e.g., real order tracking, user dashboard), consider building a **Shopify app** that exposes custom endpoints and uses the Storefront API. The current `dashboard` section can be re‑implemented as a **private app page**.
...
- [ ] CSS variable to theme setting conversion
...
**End of Guideline Plan**

*Please review the above implementation plan. Once approved, I will generate the full `guideline.md` file with all sections populated.*
